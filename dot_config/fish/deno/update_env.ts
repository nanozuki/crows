...
async function upgradeSystem(): Promise<void> {
  if (await fish.hasCommand("brew")) {
    await pkg.brewUpgrade();
  }
  if (await fish.hasCommand("paru")) {
    await pkg.paruUpgrade();
  }
  if (await fish.hasCommand("flatpak")) {
    await pkg.flatpakUpgrade();
  }
  if (await fish.hasCommand("home-manager")) {
    await pkg.homeManagerUpgrade();
  }
}
